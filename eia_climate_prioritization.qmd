...
Waffler <- function(data,
  ...
  HazardPalette1 <- PalFun(
    PalName = HazardPalette,
    N = length(hazards1),
    Names = hazards1
  )

  HazardPalette2 <- PalFun(
    PalName = HazardPalette,
    N = length(hazards2),
    Names = hazards2
  )

  HazardPalette3 <- PalFun(
    PalName = HazardPalette,
    N = length(hazards2),
    Names = hazards2
  )
...
# UI
ui <- fluidPage(
  titlePanel("Crop Hazard Visualization"),
  sidebarLayout(
    sidebarPanel(
      # Variable Selection
      selectInput("var_choice", "Choose spam variable",
        choices = c("vop", "ha"),
        selected = "vop"
      ),
      ...
      checkboxGroupInput("Regions", "Select regions",
        choices = c("Africa", "LAC", "SA", "SEA"),
        selected = c("Africa", "LAC", "SA", "SEA")
      ),

      # Country Selection (will be dynamically updated)
      uiOutput("Countries"),

      # Crop Selection (will be dynamically updated)
      uiOutput("crops"),

      # Plot Configuration
      numericInput("Crop.N1", "No. of crops (plot1)", value = 5, min = 1, max = 50),
      numericInput("Crop.N2", "No. of rows (plot2)", value = 20, min = 1, max = 50),
      numericInput("Crop.N3", "No. of crops (plot3)", value = 50, min = 1, max = 50),
      numericInput("Haz.N", "No. of hazards to show", value = 10, min = 1, max = 50),

      # Advanced Plot Settings
      selectInput("Palette", "Palette",
        choices = c("turbo", "magma", "inferno", "plasma", "viridis", "cividis", "rocket", "mako"),
        selected = "turbo"
      ),
      ...
      checkboxInput("advanced", "Show Advanced Plot Options", value = FALSE, width = NULL),
      conditionalPanel(
        "input.advanced",
...
server <- function(input, output, session) {
  ...
  crop_options <- reactive({
    ...
    if (is.null(input$Countries)) {
      return(Data[Region %in% input$Regions & (is.na(Country) | Country == ""), sort(unique(Crop))])
    } else {
  ...
  reactive({
    req(input$Waffle.Save.Plot)
    if (input$Waffle.Save.Plot == "Yes") {
      ggsave(
        filename = paste0(input$Waffle.Filename, ".png"),
